...
const Form: Component = () => {
  const { actions: { title, multiInput }, listHelpers: { multiInputHelper }, form, formState, errors, reset } = createFormActions({
    ...
    validate: (values) => {
      ...
      for (const [index, input] of values.multiInput.entries()) {
        if (input.length === 0) {
          ...
          (errs.multiInput as string[])[index] = "this input must not be left empty!";
        }
      }
      return errs;
    },
  ...
  return (
    <form ref={form} style={{ display: "flex", "flex-direction": "column" }}>
      ...
      {errors.title ? <small style={{ color: "red" }}>{errors.title}</small> : undefined}
        ...
        {(item, index) => (
          <>
            ...
            {errors.multiInput?.[index()]
              ? <small style={{ color: "red" }}>{errors.multiInput[index()]}</small>
              : undefined}
          </>
        )}
      </For>
